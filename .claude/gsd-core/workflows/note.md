...
- **Global scope**: `/home/thomas/atos/simpleEAM/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/thomas/atos/simpleEAM/.claude/notes/`)
...
2. Glob `/home/thomas/atos/simpleEAM/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/thomas/atos/simpleEAM/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/thomas/atos/simpleEAM/.claude/notes/` — works in any directory
