...
import {
  ...
  GET_SOVEREIGNTY_DATA_OBJECTS_LIST,
...
function DataObjectAnalysisCard({
  dataObject,
  onEntityClick,
}: {
  dataObject: DataObjectListItem
  onEntityClick: (ref: EntityRef) => void
}) {
...
export default function SovereigntyDataView({ onEntityClick }: SovereigntyDataViewProps) {
  ...
  const { data, loading, error } = useQuery(GET_SOVEREIGNTY_DATA_OBJECTS_LIST, {
    skip: !selectedCompanyId,
    fetchPolicy: 'cache-and-network',
  ...
  const dataObjects: DataObjectListItem[] = data?.dataObjects ?? []
