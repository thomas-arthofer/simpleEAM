...
1. **Is a shared Traefik instance from another local project (`eam.wien.arthofer.dev`) already running or expected to be reused, rather than starting a fresh dedicated one via `traefik.local.yml`?**
   ...
   - Recommendation: Plan should present both: (a) the `traefik.local.yml` snippet for a fresh dedicated instance, and (b) the `docker network connect --alias ...` command for attaching aliases to an already-running shared instance, so the developer picks whichever matches their actual setup.

2. **Should `env.template` itself be edited to change the default `TRAEFIK_CERTRESOLVER`/`TEMPORAL_UI_CERTRESOLVER` placeholder, or should the guidance stay purely in README/`.env` instance-level edits?**
   ...
   - Recommendation: Leave `env.template` defaults as illustrative placeholders (matches Phase 1 style of documenting Optional Path values without forcing them); show the concrete empty-value override only in the README Optional Path walkthrough and in this RESEARCH/PLAN's code examples.
