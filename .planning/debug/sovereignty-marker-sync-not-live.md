...
4. (G-02.2-6) Duplicating (context menu) or copy/pasting a main element with markers gives the copy its own fill/ring pair automatically, via D-02 diff detection.
   actual: |
5. Marker never appears after drop; only pressing Ctrl+R (manual sync) brings it in.
6. Marker persists after native Delete/Backspace; only Ctrl+R cleans it up.
7. Marker persists after context-menu Delete; only Ctrl+R cleans it up.
8. Duplicate/pasted copy has no ring/fill; only Ctrl+R adds it.
...
- timestamp: 2026-08-03
  checked: "Live browser instrumentation (G-02.2-4, Plan 02.2-04) — human ran the Docker-deployed client (not `yarn dev`), TEMP-DEBUG G-02.2-4 logging enabled, company 'amag' selected, native keyboard Delete on a marked main element."
  found: |
  First attempt (reported in prior turn): the running `nextgen-eam-client` container was 3+ days stale, built 2026-07-30, predating all 4 Phase 02.2 commits (including this plan's own instrumentation and Plan 02.2-03's retry-safe fix) — zero TEMP-DEBUG lines appeared for ANY action, including drag (an already-instrumented, already-working code path), confirming the container was not running the code under test at all.
  After rebuilding (`docker compose build client`) and force-recreating the container with the current commit's code, the human repeated the exact same test and reported: `handleChange entry` logged `elements.length` rising 0 -> 79 as the diagram loaded, then on delete `deletedCount` rising 0 -> 8 -> 10 across successive onChange calls; `pre-removeOrphanedMarkersLive marker states` showed 16 marker elements tracked; `removeOrphanedMarkersLive result` reported `orphansRemoved: true, tombstonedMarkers: Array(2)` (exactly the fill+ring pair for the deleted main element) on the call where `deletedCount` first became 8; `updateScene reached (orphan cleanup)` logged immediately after. The human confirmed the marker ellipse visually disappeared together with the deleted box. Separately, the human noted arrows bound to the deleted element remained on the canvas after delete, flagged as "possibly no finding."
  implication: "G-02.2-4 is CONFIRMED RESOLVED, not merely diagnosed: handleChange fires correctly on native keyboard delete, removeOrphanedMarkersLive correctly detects the tombstoned main element and cleans up exactly its own fill/ring pair, and updateScene applies the change — matching expected behavior end-to-end. The prior 'no logs appeared, even for drag' result was a false negative caused entirely by testing against a stale Docker image, not evidence of a real handleChange-never-fires bug. The arrows-left-behind observation is unrelated to sovereignty markers (arrows are not deleted when a bound shape is deleted in native Excalidraw unless explicitly re-bound/removed) and is not investigated further here."
...
Update (2026-08-03): Live browser instrumentation (see new Evidence entry above) CONFIRMS G-02.2-4 is already resolved by Plan 02.2-03's retry-safe fix — no separate bug exists in removeOrphanedMarkersLive, handleChange, or updateScene for the native-delete path. The initial "no logs appeared" result was a false negative caused by testing against a stale Docker client image/container (built 2026-07-30, predating all Phase 02.2 commits); after rebuilding and redeploying the container with current code, native delete correctly tombstones exactly the deleted element's fill/ring marker pair and the ellipse visually vanishes with the box. G-02.2-4 is CLOSED. Remaining follow-up (non-blocking, tracked outside this gap): strip the temporary `TEMP-DEBUG G-02.2-4` console.debug logging from `ExcalidrawWrapper.tsx` in a small cleanup commit. G-02.2-5 (context-menu delete) remains open/unconfirmed per the asymmetry noted above and still needs its own verification pass.
